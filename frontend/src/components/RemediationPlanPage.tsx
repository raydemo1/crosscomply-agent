...
function RemediationTaskDetail({ task, user, assignableUsers, onChanged }: TaskDetailProps): JSX.Element {
  ...
  const submit = async (): Promise<void> => {
    ...
    if (files.length === 0 && linkEvidence.length === 0) { setMessage('请至少附上一项文件或链接证据。'); return; }
    await run(async () => {
