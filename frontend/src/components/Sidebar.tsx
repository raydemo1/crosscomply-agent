  ...
  return (
    <aside className="app-sidebar sidebar">
      ...
      <div className="sidebar-user-card">
        ...
        <button type="button" className="sidebar-user-card__logout" onClick={onLogout} title="退出登录">↗</button>
      </div>
      ...
      <div className="sidebar-section">
        ...
        <div className="sidebar-scenarios">
          {SCENARIOS.map((scenario) => (
            <button key={scenario} type="button" className="sidebar-scenario" onClick={() => onScenarioClick(scenario)}>
              {scenario}
            </button>
          ))}
        </div>
      ...
      <div className="sidebar-section sidebar-history">
        <div className="sidebar-section-label">
          案件队列 <span className="sidebar-history__count">{cases.length}</span>
        </div>
        {cases.length > 0 ? (
        ...
        <div className="sidebar-history__list">
          {filtered.length === 0 ? (
            ...
            <button key={item.id} type="button" className={'history-item history-item--button' + (item.id === activeCaseId ? ' is-active' : '')} onClick={() => onOpenCase(item.id)}>
