...
def build_query_planning_messages(
    ...
    user_payload = {
        ...
        "instructions": [
            ...
            "region_condition 从 controlled_region_facets 中选择与材料事实明确匹配的中国境内地区/自贸区 facet，并用其 query_terms 组合；材料明确涉及多个地区时，为每个地区分别输出一条 region_condition；没有明确命中时不要输出 region_condition。",
            "不要因为 review_facts.regions 为 [] 就虚构 region_condition；regions 为 [] 表示材料没有可确定的具体地区。",
