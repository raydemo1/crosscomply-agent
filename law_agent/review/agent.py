...
class AgentDecision(StrictModel):
    action: Literal["read_material", "record_facts", "search_evidence", "request_input", "finish"]
...
class AgentModel:
    def __init__(self, *, model_id: str, client: OpenAICompatibleClient | None = None):
        self.node = StructuredLLMNode(
            ...
            max_retries=0, structured_output_mode="json_object",
        )
...
def run_agent(
    ...
    while state.turns < state.max_turns:
        ...
        try:
            if decision.action == "read_material":
...
def answer_agent(state: AgentState, *, gate_id: str, answer: str) -> AgentState:
    ...
    state.steps.append(AgentStep(number=state.turns, action="human_input",
                                summary="用户补充信息", observation={"answer": answer.strip()}))
