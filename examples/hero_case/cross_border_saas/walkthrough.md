...
预期输出以 `PASS case integrity` 结束。若材料哈希不一致，不要修改旧快照记录来“消除错误”，而应建立新的材料版本和快照。
...
报告渲染输入见 `report/report_data.json`，可读示例见 `report/decision_report_example.md`。正式 PDF 应至少包含案件编号、审查意见、主要法源、整改项和审批记录。

生成 PDF 后：

1. 从案件页下载正式报告；
2. 核对案件编号、审查意见、法源、整改项和审批记录与审批结果一致；
3. 将报告作为本次审批的归档材料。
